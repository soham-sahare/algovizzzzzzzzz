...
import { generateReverseStringSteps, GenericStackStep } from "@/lib/algorithms/stack/challenges";
import { Play, Pause, ArrowRightLeft } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

const CODE = `function reverse(s):
  stack = []
  for char in s:
    stack.push(char)
...
export default function ReverseStringPage() {
    ...
    const handleRun = () => {
        ...
        const generator = generateReverseStringSteps(input);
    ...
    const stepData = steps[currentStep] || { stack: [], message: "Ready", inputIndex: -1, output: "", lineNumber: undefined };

    return (
        <div className="p-8 max-w-7xl mx-auto space-y-8">
            <BackButton href="/dsa/stacks" />
            <div>
                 <h1 className="text-3xl font-bold text-foreground mb-2">Reverse String</h1>
                 <p className="text-muted-foreground">Use a Stack to reverse the order of characters.</p>
            </div>

            <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-6">
                    {/* Visualizer */}
                     <div className="bg-white dark:bg-zinc-900/50 p-8 rounded-xl border border-zinc-200 dark:border-zinc-800 min-h-[400px] flex flex-col items-center justify-start gap-12">
                        
                        {/* Input & Output */}
                        <div className="w-full grid grid-cols-2 gap-8 text-center">
                            <div>
                                <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider mb-2">Input</h3>
                                <div className="text-2xl font-mono tracking-widest">{input}</div>
                                <div className="flex justify-center mt-2 gap-1 h-2">
                                     {input.split("").map((_, i) => (
                                         <div key={i} className={`w-4 h-1 rounded-full transition-colors ${stepData.inputIndex === i ? "bg-indigo-500" : "bg-transparent"}`} />
                                     ))}
                                </div>
                            </div>
                            <div>
                                <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider mb-2">Output</h3>
                                <div className="text-2xl font-mono tracking-widest text-emerald-600 dark:text-emerald-400 min-h-[2rem]">
                                    {stepData.output}
                                </div>
                            </div>
                        </div>
                             ...
                             <AnimatePresence mode="popLayout">
                                {stepData.stack.map((char, i) => (
                                    <motion.div
                                        ...
                                        exit={{ opacity: 0, x: 50 }}
                                        className={`w-full h-10 mb-1 rounded flex items-center justify-center font-bold shadow-sm border bg-white border-zinc-200 dark:bg-zinc-800 dark:border-zinc-700 text-foreground`}
                                    >
                                        {char}
                                    </motion.div>
                                ))}
