...
import { generateCircularEnqueueSteps, generateCircularDequeueSteps } from "@/lib/algorithms/queue/circularQueue";
...
export default function CircularQueuePage() {
    ...
    const handleCreate = () => {
        ...
        setSize(0);
        setSteps([]);
        setMessage(`Circular Queue reset (Capacity: ${capacity})`);
    };
    ...
    const handleEnqueue = () => {
        ...
        executeOperation(generateCircularEnqueueSteps(array, front, rear, val, capacity, size));
    };

    const handleDequeue = () => {
        setActiveCode(DEQUEUE_CODE);
        executeOperation(generateCircularDequeueSteps(array, front, rear, capacity, size));
    };
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-8">
            ...
            <div>
                 <h1 className="text-3xl font-bold text-foreground mb-2">Circular Queue (Ring Buffer)</h1>
                 <p className="text-muted-foreground">Efficiently utilizes space by connecting the end back to the start.</p>
            </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white dark:bg-zinc-900/50 rounded-xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden min-h-[400px] flex items-center justify-center relative flex-col">
                        ...
                        {/* Circular Representation (Simplified as Linear with wrapping arrow hints, or just distinct styling) */}
                        {/* We'll stick to linear to keep it readable, but maybe add a visual "Link" from end to start if needed? */}
                        {/* Actually, let's just arrange them normally and let the pointers jump. */}
                        ...
                        <div className="flex gap-1 relative pl-8 pr-8">
                            <AnimatePresence>
                                {array.map((value, index) => {
                                    ...
                                    const isActive = value !== null; // In circular, active is where Value is present
                        ...
                        <div className="mt-8 text-xs text-zinc-400">
                             Indices wrap around: {capacity - 1} ➔ 0
                        </div>
                    </div>
