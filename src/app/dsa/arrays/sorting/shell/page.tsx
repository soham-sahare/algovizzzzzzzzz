...
const shellSortCode = `function shellSort(arr):
  gap = n/2
  while gap > 0:
    for i = gap to n:
      temp = arr[i]
...
export default function ShellSortPage() {
  ...
  const generateRandomArray = () => {
    ...
    // Pre-calculate steps
    const generator = generateShellSortSteps([...newArray]);
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div>
        <h1 className="text-3xl font-bold text-foreground mb-2">Shell Sort</h1>
        <p className="text-muted-foreground">In-place comparison sort. Generalized version of insertions sort that allows exchange of far items.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <CodeHighlight code={shellSortCode} activeLine={currentStepData.lineNumber} />
           ...
           <div className="bg-zinc-50 dark:bg-zinc-900/30 p-5 rounded-xl border border-zinc-200 dark:border-zinc-800 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <span className="font-mono bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 px-2 py-0.5 rounded">O(n log n) - O(n²)</span>
                    </div>
                    <div>
                ...
                <p className="text-xs text-muted-foreground mt-2">Depends on gap sequence. Best known is O(n log²n).</p>
           </div>
        </div>
      </div>
    </div>
