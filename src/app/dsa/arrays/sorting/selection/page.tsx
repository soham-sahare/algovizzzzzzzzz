...
import { generateSelectionSortSteps, SortingStep } from "@/lib/algorithms/sorting/selectionSort";
import { Play, Pause, RotateCcw, FastForward } from "lucide-react";
import BackButton from "@/components/ui/BackButton";

const selectionSortCode = `for i = 0 to n-1
  min = i
  for j = i+1 to n
    if arr[j] < arr[min]
      min = j
...
export default function SelectionSortPage() {
  ...
  const generateRandomArray = () => {
    ...
    setArray(newArray);
    const generator = generateSelectionSortSteps(newArray);
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           <h1 className="text-3xl font-bold text-foreground mb-2">Selection Sort Visualization</h1>
           <p className="text-muted-foreground">Repeatedly finds the minimum element from the unsorted part and puts it at the beginning.</p>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
            <div>
                ...
                <CodeHighlight code={selectionSortCode} activeLine={currentStepData.lineNumber} />
            </div>
