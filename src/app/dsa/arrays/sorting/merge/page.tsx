...
import { SortingStep } from "@/lib/algorithms/sorting/selectionSort"; // Reuse type
...
export default function MergeSortPage() {
  ...
  const generateRandomArray = () => {
    setIsPlaying(false);
    setCurrentStep(0);
    const newArray = Array.from({ length: 30 }, () => Math.floor(Math.random() * 90) + 10);
    setArray(newArray);
    const generator = generateMergeSortSteps(newArray);
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           <h1 className="text-3xl font-bold text-foreground mb-2">Merge Sort Visualization</h1>
           <p className="text-muted-foreground">Divide and conquer algorithm that divides the array into halves, sorts them, and merges them back together.</p>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
            <div>
                ...
                <CodeHighlight code={mergeSortCode} activeLine={currentStepData.lineNumber} />
            </div>
             ...
             <div className="bg-white dark:bg-zinc-900/30 p-5 rounded-xl border border-zinc-200 dark:border-zinc-800 text-sm shadow-sm dark:shadow-none">
                ...
                <div className="space-y-3 font-mono">
                    <div className="flex justify-between items-center">
                        ...
                        <span className="text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-400/10 px-2 py-0.5 rounded">O(n log n)</span>
                    </div>
                    <div className="flex justify-between items-center">
                        <span className="text-zinc-600 dark:text-zinc-500">Average Case</span>
                        <span className="text-yellow-600 dark:text-yellow-400 bg-yellow-100 dark:bg-yellow-400/10 px-2 py-0.5 rounded">O(n log n)</span>
                    </div>
                    <div className="flex justify-between items-center">
                        <span className="text-zinc-600 dark:text-zinc-500">Worst Case</span>
                        <span className="text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-400/10 px-2 py-0.5 rounded">O(n log n)</span>
                    </div>
                </div>
            </div>
        </div>
